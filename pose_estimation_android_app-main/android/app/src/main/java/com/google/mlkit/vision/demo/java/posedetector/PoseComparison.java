...
public class PoseComparison {
     ...
     *         the defined poses.
     */
    public float[] comparePose(int[] angleList){
        float angleErrors[][];
        int[][] referenceAngles;
        int error;
        int difference;
        ...
        for(i=0; i<referenceAngles.length; i++){
            ...
            for(j=0; j<10; j++){
                ...
                difference= (int) Math.abs(angleList[j]-referenceAngles[i][j]);
                //error = (float) (100/(1 +Math.pow(Math.E,((-Math.E)*difference+(Math.pow(Math.E,2))))));
                error= (int) Math.abs(angleList[j]-referenceAngles[i][j]);
...
//            }
     ...
     */
    private int[][] getReferenceeAngles(){
        int referenceAngles[][] = {{180}};
